...
all-debug:
	mkdir -p $(BINDIR)
	CGO_ENABLED=0 go build $(DEBUG_GCFLAGS) -o $(LOG_COURIER_BIN) $(LOG_COURIER_CMDDIR)/.
	CGO_ENABLED=0 go build $(DEBUG_GCFLAGS) -o $(ENSURE_SERVICE_USER_BIN) $(ENSURE_SERVICE_USER_CMDDIR)/.
