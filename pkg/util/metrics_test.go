...
var _ = Describe("StartMetricsServerIfEnabled", func() {
	...
	Context("when metrics server is enabled", func() {
		...
		It("should respond to /healthcheck endpoint with 200 and no body", func() {
			configSpec.Set("test-metrics.enabled", true)
			configSpec.Set("test-metrics.listen-address", "127.0.0.1")
			configSpec.Set("test-metrics.listen-port", 0)
		...
		It("should return 405 for non-GET methods on /healthcheck", func() {
			configSpec.Set("test-metrics.enabled", true)
			configSpec.Set("test-metrics.listen-address", "127.0.0.1")
			configSpec.Set("test-metrics.listen-port", 0)
			...
			resp, err := http.Post(fmt.Sprintf("http://%s/healthcheck", server.Addr), "text/plain", nil)
			Expect(err).ToNot(HaveOccurred())
			defer func() {
				_ = resp.Body.Close()
			}()
