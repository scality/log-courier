...
type Config struct {
	...
	// OffsetFlushTimeThreshold is the time threshold for offset flushing
	OffsetFlushTimeThreshold time.Duration
	...
	for _, batch := range batches {
		...
		go func() {
			...
			if err := p.processLogBatchWithRetry(ctx, batch); err != nil {
				...
				// Record lost records for permanent errors
				if isPermanent {
					p.metrics.General.RecordsLost.Add(float64(batch.LogCount))
				}
...
func (p *Processor) uploadLogBatch(ctx context.Context, batch LogBatch) (*ProcessResult, error) {
	...
	p.metrics.Fetch.BatchSize.Observe(float64(len(records)))
