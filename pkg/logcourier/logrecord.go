...
type LogRecord struct {
	...
	Timestamp           time.Time `ch:"timestamp"`           // Event timestamp (for ordering, key generation)
	...
	RaftSessionID       uint16    `ch:"raftSessionID"`       // Bucket raft session ID (for ordering, offset tracking)
}
