...
func (f *FailingOffsetManager) CommitOffset(ctx context.Context, bucket string, raftSessionID uint16, insertedAt time.Time, timestamp time.Time, reqID string) error {
	count := f.commitCount.Add(1)
	if count <= f.failUntilCount {
		return fmt.Errorf("simulated offset commit failure (attempt %d)", count)
	}
	return f.manager.CommitOffset(ctx, bucket, raftSessionID, insertedAt, timestamp, reqID)
}

// GetOffset delegates to the underlying manager
func (f *FailingOffsetManager) GetOffset(ctx context.Context, bucket string, raftSessionID uint16) (logcourier.OffsetTimestamps, error) {
